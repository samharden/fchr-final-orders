...
SNAP_DIR=angular-finalordercat-snapshots
...
do
  ...
  if [[ $i = 0 ]]; then
    ...
    rm -rf $SNAP_DIR/step-$i/app/finalorders
  fi

  if [[ $i = 5 ]]; then
    cp -r app/img $SNAP_DIR/step-0/app/
    cp -r app/finalorders $SNAP_DIR/step-0/app/
  fi
done
...
zip -r angular-finalordercat-`date +"%y%m%d_%H%M"` $SNAP_DIR
